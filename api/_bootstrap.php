...
function macemay_is_payable_cart_item(array $item): bool
{
    return macemay_is_payable_plate_item($item) || macemay_is_payable_boutique_item($item);
}
...
function macemay_normalize_cart_items(array $items): array
{
    if (!$items) {
        macemay_json_response(['error' => 'Panier vide'], 400);
    }
    ...
    foreach ($items as $item) {
        ...
        if (!macemay_is_payable_cart_item($item)) {
            macemay_json_response(['error' => 'Panier contient des articles non payables en ligne'], 400);
        }
        ...
        $type = (string)($item['type'] ?? '');
        if ($type === '') {
            $type = macemay_is_payable_boutique_item($item) ? 'boutique' : 'plaque';
        }
